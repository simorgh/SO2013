...
static void liberar_proceso(){
	...
	eliminar_elem(&lista_listos,p_proc_actual);  /* proc. fuera de listos */
	 ...
	 /* Si un procés sobreviu al seu pare, queda orfe i l'identificador del seu procés pare passa a ser -1.
	...
	for(i=0;i<MAX_PROC;i++){
		if(tabla_procs[i].estado != NO_USADA && tabla_procs[i].p_id == p_proc_actual->id ){
			 tabla_procs[i].p_id = -1; 	/* proceso huérfano, el padre pasa a ser init (-1) */
		}
	}
...
int sis_fijar_prio(){
	...
	if(p_proc_actual->prioridad_efectiva < PEprevia){
		...
		if(maxima_prioridad(&lista_listos) > p_proc_actual->prioridad_efectiva){
