...
static BCP * maxima_prioridad (lista_BCPs *lista){
	...
	printk("\n_-_-_-_-_-_-_\n ID:\t%d\n P_ID:\t%d\n PB:\t%d\n PE:\t%d\n Estat:\t%d\n_-_-_-_-_-_-_\n", max->id, max->p_id, max->prioridad, max->prioridad_efectiva, max->estado); 
	return max;
}
...
/*
 * Función auxiliar que busca un proceso en una lista, devuelve TRUE si se ha encontrado,
 * en caso opuesto FALSE.
 * NOTA: Utilizada en la práctica para saber si un proceso está o no en lista_espera.
 *
 */
static boolean buscar_elem(lista_BCPs *lista, BCP * proc){
	BCP* p;
	p = lista->primero;
	
	printk("\n-> Buscando Proceso en lista_espera...");
	while(p != NULL){
	    if(p->id == proc->id){
	    	printk("\n-> ^^^^^ Proceso encontrado en lista_espera. ^^^^^");
	    	return TRUE;
	    }
	    p = p->siguiente;
	}
	printk("\n->  ^^^^^ El proceso NO está en lista_espera. ^^^^");
	return FALSE;	
}
...
static void liberar_proceso(){
	...
	eliminar_elem(&lista_listos,p_proc_actual);  /* proc. fuera de listos */
	 ...
	 /* Si un procés sobreviu al seu pare, queda orfe i l'identificador del seu procés pare passa a ser 0.
	 * Així doncs, quan un procés acaba caldrà verificar l'existència de fills i actualitzar els corresponents camps.*/
	int i;
	for(i=0;i<MAX_PROC;i++){
		if(tabla_procs[i].estado != NO_USADA && tabla_procs[i].p_id == p_proc_actual->id ){
			tabla_procs[i].p_id = 0; 	/* proceso huérfano, el padre pasa a ser init (id 0) */
			tabla_procs[0].fills++;		// actualizamos los hijos de init
		}
	}
...
static void muestra_lista(lista_BCPs* lista){
	...
	while(actual){
		printk("______________\n ID:\t%d\n P_ID:\t%d\n PB:\t%d\n PE:\t%d\n Estat:\t%d\n______________\n", actual->id, actual->p_id, actual->prioridad, actual->prioridad_efectiva, actual->estado);
...
static void desbloquear (BCP * proc, lista_BCPs * lista){
	...
	if( p_proc_actual->estado==EJECUCION &&  proc->prioridad_efectiva > p_proc_actual->prioridad_efectiva){
		/* Activem interrupció software*/
		replanificacio_pendent = TRUE; 
		activar_int_SW();
	}
...
static int crear_tarea(char *prog){ /*MODIFICADA : deshabilitem interrupcions de rellotge */
	...
	if (imagen){
		...
		if(!p_proc_actual){
			/* El procés init és l'únic que no té pare i per tant l'identificador del seu procés pare, per convenció, és -1. */
			p_proc->p_id = -1;
			...
			p_proc->p_id = p_proc_actual->id;
			p_proc_actual->fills++;  /* actualizamos el numero de hijos del padre (incremento). */
 ...
 * tratar_padre
 * -----------------------------------------------------------------------------------------
 * Funció que crida a desbloquear quan un procés pare tingui tots els seus fills finalitzats.
 * Rep com a paràmetre el pare per aprofitar el recorregut ja realitzat a liberar_proceso.
 */
static void tratar_padre(){
	printk("-> TRATAR PADRE\n");

	/* actualizamos (decremento) el num de hijos del padre del proceso a eliminar */
	tabla_procs[p_proc_actual->p_id].fills--;

	/* si el proceso esta en lista de espera, lo tratamos */
	if(buscar_elem(&lista_espera, &tabla_procs[p_proc_actual->p_id])==TRUE) {
	
		/* comprobamos que el proceso ya no tiene hijos */
		if(tabla_procs[p_proc_actual->p_id].fills == 0){
			/* Un procés es desbloquejarà d'aquesta llista només si tots els seus fills han acabat l'execució
			 * passarà a lista_listos incrementant la seva prioritat efectiva en un 10%.*/
			tabla_procs[p_proc_actual->p_id].prioridad_efectiva *= 1.1;
			desbloquear(&tabla_procs[p_proc_actual->p_id] ,&lista_espera);
		}
...
int sis_fijar_prio(){
	...
	unsigned int PEprevia = p_proc_actual->prioridad_efectiva;
	...
	/* Controlem que la nova prioritat EFECTIVA fixada és igual o superior a l'actual (el procés que s'executa té màxima prioritat).
	  * si la prioritat EFECTIVA a assignar és inferior cal que el planificador revisi si tenim altres processos amb prioritat superior. (CCI) */
	if(p_proc_actual->prioridad_efectiva < PEprevia){
		/* Comprovació extra, només activarem la INT_SW si existeix un procés a listos DIFERENT a l'actual */
		if( (maxima_prioridad(&lista_listos)->prioridad_efectiva) > p_proc_actual->prioridad_efectiva){
...
/*
 * Tratamiento de llamada al sistema get_ppid. (Parent Process Identifier)
 */
int sis_get_ppid(){
	printk("-> GET_PPID: Proceso Padre %d\n", p_proc_actual->p_id);
	return p_proc_actual->p_id;
}

/*
 * Tratamiento de llamada al sistema espera.
 * ---------------------------------------------------------------------
 * Retorna 0 quan s'han acabat d'executar tots els fills d'un proces i -1 si en fer la crida no en té.
 */
 int sis_espera(){
 	if(p_proc_actual->fills == 0) return -1;  /* si el procés no té fills */

 	printk("|==============| Comienza Espera ID: %d |==============|\n", p_proc_actual->id);
	bloquear(&lista_espera);
	printk("|==============| Espera ID: %d Finalizada|==============|\n", p_proc_actual->id);
 	return 0; /* s'han acabat d'executar tots els seus fills */
 }
...
int main(){
	...
	if (crear_tarea((void *)"init")<0) panico("no encontrado el proceso inicial");
